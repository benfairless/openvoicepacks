...
def update(command):
    """Update dependencies."""
    command.run("uv lock --upgrade", pty=True)
...
def clean(command):
    ...
    command.run("rm -rf dist/ .pytest_cache/")
...
def docker(command):
    ...
    command.run("docker build -t openvoicepacks:latest .")
...
def build(command, wheel=False, sdist=False):
    """Build distributable. Defaults to building both sdist and wheel."""
    cmd = "uv build"
    if wheel:
        cmd += " --wheel"
    if sdist:
...
def version(command, short=False):
